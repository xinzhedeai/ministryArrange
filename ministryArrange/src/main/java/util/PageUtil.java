...
public class PageUtil {
	...
	public static LayUIGridObj searchByPage(Object object, Map paramMap, String method) throws SysException {
		int pageNum = 0;
		int pageSize = 0;
//		EasyUIGridObj easyUIGridObj = new EasyUIGridObj();
		LayUIGridObj layUIGridObj = new LayUIGridObj();
		try {
			pageNum = Integer.parseInt((String) paramMap.get("page"));
			pageSize = Integer.parseInt((String) paramMap.get("limit"));
		} catch (Exception e) {
		...
		if(page.getTotal() > (pageNum - 1) * pageSize) {
			layUIGridObj.setData(page.getResult());
			layUIGridObj.setCount(page.getTotal());
		} else {
			layUIGridObj.setData(null);
			layUIGridObj.setCount(page.getTotal());
		}
